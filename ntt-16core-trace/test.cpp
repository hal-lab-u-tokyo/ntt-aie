...
int main(int argc, const char *argv[]) {
  ...
  constexpr int n_row = 4;
  int n_cores = n_column * n_row;
  int in_per_core = N / n_cores;
  int intotal_per_core = N + in_per_core;
  ...
  auto bo_inA = xrt::bo(device, intotal_per_core * n_cores * sizeof(int32_t), XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(2));
  auto bo_outC = xrt::bo(device, N * sizeof(int32_t), XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(5));
  ...
  root[0] = 1;
  make_roots(N, root, p, g);
  for (int i = 0; i < n_cores; i++){
    // Input Data
    int32_t *base = bufInA + intotal_per_core * i;
    for (int j = 0; j < in_per_core; j++){
      base[j] = i * in_per_core + j;
    }
    // Root
    base += in_per_core;
    memcpy(base, root.data(), N * sizeof(int));
  }
  ...
  if (trace_size > 0) {
    ...
    test_utils::write_out_trace(((char *)bufOut) + N * sizeof(int32_t), trace_size,
